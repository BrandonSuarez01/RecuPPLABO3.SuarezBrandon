...
window.addEventListener("click", (e)=>{
    if(e.target.matches("td")){
        ...
        setTimeout(()=>{
            mensajePerso("Se Elimino el anuncio correctamente!!!", "red");
        }, 3000);
        limpiarForm();
    }
...
$formulario.addEventListener("submit", (e)=>{
    e.preventDefault();
    const {txtId, txtTitulo, txtDescri, rdoTipo, txtPrecio, txtRaza, txtFecha, txtVacunas, rdoPosee} = $formulario;
    let dato = "";
    ...
    if(validarCampoVacio([txtTitulo, txtDescri, txtPrecio, txtFecha, txtVacunas, dato, rdoPosee]) == true && validarCaracteres([txtTitulo, txtDescri]) && validarPrecio(txtPrecio)){
        
        const nuevoAnuncio = new Anuncio_Mascota(txtId.value, txtTitulo.value, txtDescri.value, rdoTipo.value, txtPrecio.value, txtRaza.value, txtFecha.value, txtVacunas.value, dato, rdoPosee.value);
        if(nuevoAnuncio.id == ""){ 
            ...
            limpiarForm();
            setTimeout(()=>{
                mensajePerso("Se añadio el anuncio correctamente!!!", "green");
            }, 3000);
        }
        else{
            ...
            setTimeout(()=>{
                mensajePerso("Se modifico el anuncio correctamente!!!", "orange");
            }, 3000)
            limpiarForm();
        }
    }
...
//FUNCION MENSAJE PROPIA:
function mensajePerso(texto, color){

    const mensaje = document.createElement("h2");
    mensaje.textContent = texto;
    $divMensaje.appendChild(mensaje);
    $divMensaje.style.display = 'block';
    $divMensaje.style.background = color;
